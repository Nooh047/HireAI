...
Score: ${candidate['score']}''',
                          style: const TextStyle(
                            color: AppTheme.textSecondary, // Use theme secondary text color
                            fontSize: 14,
                          ),
                        ),
                      ),
                      trailing: Row(
                        ...
                        children: [
                          TextButton.icon(
                            onPressed: () {
                              _openResumeFile(context, candidate['id']);
                            },
                            icon: const Icon(Icons.remove_red_eye, color: AppTheme.textPrimary),
                            label: const Text(
                              "Preview",
                          ...
                          IconButton(
                            ...
                            onPressed: () {
                              _downloadResume(context, candidate['id'], candidate['name']);
                            },
                          ),
                        ],
                      ),
  ...
  Future<void> _downloadResume(BuildContext context, int resumeId, String candidateName) async {
    try {
      final String resumeUrl = "http://127.0.0.1:5000/resume/$resumeId";
      final response = await http.get(Uri.parse(resumeUrl));

      if (response.statusCode == 200) {
        final Directory tempDir = await getApplicationDocumentsDirectory();
        final File file = File('${tempDir.path}/$candidateName.pdf');
